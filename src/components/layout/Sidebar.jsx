import { Bell, Compass, Home, Moon, PenSquare, Sun, User } from "lucide-react";
...
function Sidebar() {
  ...
  return (
    <aside className="sticky top-0 hidden h-screen w-[272px] flex-col justify-between border-r border-[var(--border)] bg-[var(--surface)] p-4 md:flex">
      ...
      {/* Theme toggle — icon shows what you'll switch TO */}
      <button
        onClick={toggleTheme}
        className="flex items-center justify-between rounded-[8px] border border-[var(--border)] bg-[var(--card)] px-4 py-3 text-sm text-[var(--text-primary)] hover:bg-[var(--surface)] transition-colors"
      >
        <span>
